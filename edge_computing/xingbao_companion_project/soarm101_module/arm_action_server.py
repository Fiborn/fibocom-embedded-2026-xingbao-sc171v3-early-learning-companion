...
动作映射:
  nod        -> group_1 (1->2->1->2->1->2->0, smoothstep)
  shake_head -> group_2 (3->4->3->4->3->4->0, smoothstep)
...
JOINT_POSES: dict[int, dict[str, float]] = {
    ...
    2: {
        "shoulder_pan.pos": -2.2857142857142856,
        "shoulder_lift.pos": -46.989010989010985,
...
def move_to_action(robot, target: dict[str, float], steps: int,
                   step_delay: float, use_smoothstep: bool = False):
    """移动到目标姿态.
...
class ArmActionServer:
    ...
    def handle_action_sync(self, action_name: str, request_id: str = "") -> dict:
        ...
        # 1. 映射外部动作名到内部组名
        group = ACTION_MAP.get(action_name, action_name)

        # 2. 白名单校验 — 未知动作降级为 stay_still
