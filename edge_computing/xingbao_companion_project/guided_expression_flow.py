...
WAIT_PROACTIVE_DEMO_REPLY = "wait_proactive_demo_reply"
...
class DinosaurExpressionFlow:
    ...
    def start_proactive_demo(self) -> GuidedExpressionReply:
        """Open the fixed proactive-companionship line for the finals demo.

        The future touch button owns the explicit enable action and five-second
        wait. This flow intentionally adds no timer or autonomous policy.
        """
        with self._lock:
            self._reset_locked()
            self._stage = WAIT_PROACTIVE_DEMO_REPLY
            return self._reply(
                "小宇，我注意到你安静了一会儿。你好像有点闷闷的，旁边还有你的水杯。要不要先喝一口水，再和我聊聊天呀？",
                scene="proactive_demo_opening",
    ...
    def recovery_input_for_current_step(self) -> str | None:
        ...
        with self._lock:
            return {
                ...
                WAIT_PROACTIVE_DEMO_REPLY: "我想和你聊聊天。",
            }.get(self._stage)
    ...
    def _start_if_matched(
        ...
        if activation_score < ACTIVATION_SIMILARITY_THRESHOLD:
            return None
        self._nickname = nickname_match.group("name").strip()
    ...
    def _advance(self, compact: str) -> GuidedExpressionReply | None:
        if self._stage == WAIT_PROACTIVE_DEMO_REPLY:
            self._reset_stage_only_locked()
            return self._reply(
                "好的，你的名字叫什么？你有什么爱好呀？",
                scene="proactive_demo_profile_prompt",
    ...
    def _complete_recast_locked(self) -> GuidedExpressionReply:
        ...
        self._recast_seen_features.clear()
        return self._reply(
            "你自己说得真完整！我也记住你喜欢腕龙啦。来，和我的钳子轻轻击个掌庆祝吧！",
